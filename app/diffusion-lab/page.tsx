...
export default function DiffusionLabPage() {
  ...
  const [result, setResult] = useState<{ imageUrl: string; processingTime: number; timestamp?: number } | null>(null);
  const [devMode, setDevMode] = useState(false);
  const [activeSection, setActiveSection] = useState("diffusion");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/generate-diffusion', {
        method: 'POST',
        ...
        <div className="max-w-3xl mx-auto">
          ...
          >
            ...
            <p className="text-muted-foreground font-sans text-sm max-w-xl mx-auto">
              Noise-to-image generation via iterative denoising
            </p>
          </motion.div>
          ...
          >
            <div className="p-4 border-b border-border/30">
              ...
              <p className="text-xs text-muted-foreground mt-2 font-sans">
                Generates anime images from pure noise through progressive denoising steps
              </p>
            </div>
            ...
            <div className="aspect-square relative bg-background/30">
              {isGenerating ? (
                ...
                <div className="relative h-full">
                  <img 
                    src={`${result.imageUrl}?t=${result.timestamp || Date.now()}`} 
                    alt="Diffusion output" 
                    className="w-full h-full object-contain"
                    key={result.imageUrl + (result.timestamp || Date.now())}
                  />
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-xs text-muted-foreground font-sans">Click Generate to create an image...</p>
                  </div>
                </div>
              )}
            </div>
